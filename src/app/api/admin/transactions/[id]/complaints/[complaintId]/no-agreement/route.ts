...
import { complaintNoAgreementSchema } from "@/server/complaint/contracts";
import { recordNoAgreement } from "@/server/complaint/service";
...
export async function POST(request: Request, context: { params: { id: string; complaintId: string } }) {
  try {
    ...
    const body = complaintNoAgreementSchema.parse(await request.json());
    return NextResponse.json(await recordNoAgreement(
      account, context.params.id, context.params.complaintId, body,
      { key, requestHash: hashRequest(body) }
    ));
  } catch (error) {
    const message = error instanceof Error ? error.message : "";
    const status = message === "UNAUTHENTICATED" ? 401 : message === "FORBIDDEN" || message.includes("ASSIGNMENT") ? 403 : message.includes("VERSION") ? 409 : 400;
    return NextResponse.json({ message: "Hasil tanpa kesepakatan belum dapat dicatat." }, { status });
  }
}
